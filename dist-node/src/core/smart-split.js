 ...
 */
function buildContextAnalysisPrompt(files, overallDiff) {
 ...
 */
function buildFallbackPrompt(files) {
 ...
 */
export async function analyzeFileContext(files, overallDiff, config) {
    try {
        ...
        try {
            ...
            try {
                ...
                // Tentar uma abordagem mais agressiva - extrair apenas a estrutura básica
                try {
                    // Procurar por padrões de grupos no JSON truncado
                    const groupMatches = jsonStr.match(/"id"\s*:\s*"[^"]*"[^}]*}/g);
                    if (groupMatches && groupMatches.length > 0) {
                        log.warn('⚠️ Tentando reconstruir JSON a partir de grupos encontrados');
                        const groups = [];
                        for (const match of groupMatches) {
                            try {
                                // Tentar extrair informações básicas de cada grupo
                                const idMatch = match.match(/"id"\s*:\s*"([^"]*)"/);
                                const nameMatch = match.match(/"name"\s*:\s*"([^"]*)"/);
                                const descMatch = match.match(/"description"\s*:\s*"([^"]*)"/);
                                const filesMatch = match.match(/"files"\s*:\s*\[([^\]]*)\]/);
                                const confidenceMatch = match.match(/"confidence"\s*:\s*([0-9.]+)/);
                                if (idMatch && nameMatch && filesMatch) {
                                    ...
                                    groups.push({
                                        id: idMatch[1],
                                        name: nameMatch[1],
                                        description: descMatch ? descMatch[1] : 'Grupo extraído',
                                        files: files,
                                ...
                                log.warn(`⚠️ Erro ao processar grupo: ${groupError}`);
                            }
                        }
                        if (groups.length > 0) {
                            analysis = { groups };
                        ...
                        // Tentar uma abordagem ainda mais específica para o caso atual
                        // O JSON parece ter 3 grupos válidos mas está truncado
                        log.warn('⚠️ Tentando extrair grupos do JSON truncado usando regex específico');
        ...
        const allGroupedFiles = analysis.groups.flatMap((group) => group.files || []);
 ...
 */
export async function generateGroupDiff(group) {
 ...
 */
export async function handleSmartSplitMode(gitStatus, config, args) {
    if (!args.silent) {
    ...
    const analysis = await analyzeFileContext(gitStatus.stagedFiles, gitStatus.diff, config);
    if (!analysis.success) {
        if (!args.silent) {
        ...
        if (args.yes) {
            ...
            if (group.files.length === 1 && group.files[0]) {
                ...
                try {
                    ...
                    execSync(`git commit ${filesArg} -m ${escapedMessage}`, {
                        stdio: 'pipe',
                    });
            ...
            // Modo interativo
            const { showCommitPreview, editCommitMessage, copyToClipboard, showCancellation, } = await import('../ui/index');
