 ...
 */
export function buildPrompt(diff, config, filenames) {
 ...
 */
function getStyleInstructions(style, language) {
    const instructions = {
        pt: {
 ...
 */
export function processOpenAIMessage(message) {
 ...
 */
export async function generateCommitMessage(diff, config, filenames) {
    try {
 ...
 */
export async function generateWithRetry(diff, config, filenames, maxRetries = 3) {
    let lastError = '';
